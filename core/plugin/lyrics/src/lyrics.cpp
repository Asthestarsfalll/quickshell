...
double candidateScore(const QString &artist, const QString &title, const QString &album,
                      double duration, const QVariantMap &candidate)
{
...
QVariantMap candidateWithIdentity(const QString &provider, const QString &id,
...
bool isMetadataLine(const QString &line)
{
    static const QRegularExpression metadata(
        QStringLiteral(R"(^\[(?:ar|ti|al|by|re|ve|offset)\s*:)"),
                      ...
                      const QString &playerId)
{
    ...
    if (m_artist == nextArtist && m_title == nextTitle && m_album == nextAlbum
        ...
        && qFuzzyCompare(m_duration + 1.0, nextDuration + 1.0)) {
        return;
    }
...
{
    if (m_artist.isEmpty() && m_title.isEmpty() && m_album.isEmpty()
        && m_playerId.isEmpty() && !m_loading && !m_reply)
        return;
    ...
    for (const QString &line : lines) {
        ...
        if (!timestamps.isEmpty()) {
            ...
            for (const double time : timestamps)
                result.append(QVariantMap{{QStringLiteral("time"), time},
                                          {QStringLiteral("text"), lyricText}});
            continue;
        }
        ...
        if (lyricText.isEmpty() || isMetadataLine(trimmed) || looksLikeMalformedTimestamp(trimmed))
            continue;
        result.append(QVariantMap{{QStringLiteral("time"), -1.0},
                                  {QStringLiteral("text"), lyricText}});
    }
...
bool Lyrics::loadLocalLyrics(quint64 generation)
{
    ...
    QVariantMap candidate = candidateWithIdentity(QStringLiteral("Local"), path,
                                                   m_title, m_artist, m_album, m_duration);
...
bool Lyrics::loadCachedLyrics(quint64 generation)
{
    ...
    QVariantMap candidate = candidateWithIdentity(provider, id,
                                                   content.value(QStringLiteral("title")).toString(),
                                                   content.value(QStringLiteral("artist")).toString(),
                                                   content.value(QStringLiteral("album")).toString(),
...
bool Lyrics::acceptRawLyrics(const QString &provider, const QString &synced,
...
void Lyrics::startReply(const QUrl &url, ReplyKind kind, quint64 generation)
{
    ...
    connect(reply, &QNetworkReply::finished, this,
            [this, guardedReply, kind, generation]() {
                if (guardedReply)
...
{
    const QString synced = firstString(json, {QStringLiteral("syncedLyrics"),
    ...
    if (candidate.value(QStringLiteral("id")).toString().isEmpty() && !id.isEmpty())
        candidate.insert(QStringLiteral("id"), id);
    candidate.insert(QStringLiteral("title"), firstString(json, {QStringLiteral("trackName"), QStringLiteral("name")}));
    candidate.insert(QStringLiteral("artist"), firstString(json, {QStringLiteral("artistName"), QStringLiteral("artist")}));
    candidate.insert(QStringLiteral("album"), firstString(json, {QStringLiteral("albumName"), QStringLiteral("album")}));
...
QVariantList Lyrics::parseLrclibCandidates(const QJsonDocument &document) const
{
    ...
    for (const QJsonValue &value : document.array()) {
        ...
        const QString synced = firstString(item, {QStringLiteral("syncedLyrics"),
                                                   QStringLiteral("synced_lyrics")});
        const QString plain = firstString(item, {QStringLiteral("plainLyrics"),
                                                  QStringLiteral("plain_lyrics")});
        if (id.isEmpty() || (synced.isEmpty() && plain.isEmpty()))
            continue;

        QVariantMap candidate = candidateWithIdentity(
            QStringLiteral("LRCLIB"), id,
...
QVariantList Lyrics::parseNetEaseCandidates(const QJsonDocument &document) const
{
    ...
    const QJsonArray songs = document.object().value(QStringLiteral("result"))
                                 .toObject().value(QStringLiteral("songs")).toArray();
...
void Lyrics::cacheLyrics(const QString &provider, const QString &id,
...
QString Lyrics::cacheIndexPath() const
{
    return cacheDirectory() + QStringLiteral("/index.json");
}
...
{
    const QByteArray identity = (normalized(m_artist) + QLatin1Char('\n')
                                 + normalized(m_title) + QLatin1Char('\n')
                                 + normalized(m_album) + QLatin1Char('\n')
                                 + QString::number(qRound64(m_duration * 1000.0))).toUtf8();
...
QString Lyrics::findLocalLyricsPath(const QString &directory) const
{
    ...
    const QString canonicalBase = artistPart.isEmpty() ? titlePart : artistPart + QStringLiteral(" - ") + titlePart;
    ...
    while (iterator.hasNext()) {
        ...
        if (fallback.isEmpty() && !artistPart.isEmpty()
            && base.contains(comparable(artistPart)) && base.contains(comparable(titlePart)))
            fallback = info.absoluteFilePath();
    }
    return fallback;
}
