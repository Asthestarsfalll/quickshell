...
def prepare_niri_profile(paths: ClavisPaths, release: Path) -> Path:
    ...
    generated_layers = _existing_layers(
        [
            base,
    ...
    outputs = paths.generated_home / "niri/outputs.kdl"
    escaped_outputs = str(outputs).replace("\\", "\\\\").replace('"', '\\"')
    lines.append(f'include optional=true "{escaped_outputs}"\n')
    for layer in _existing_layers([compatibility_override, profile_override]):
        escaped = str(layer).replace("\\", "\\\\").replace('"', '\\"')
        lines.append(f'include "{escaped}"\n')
