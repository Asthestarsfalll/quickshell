...
Singleton {
    ...
    readonly property string globalDesktopLayoutMode:
        state.globalDesktopLayoutMode
    ...
    function setCardEnabled(cardId, enabled) {
        return root.commit(
            CardState.setEnabled(root.internalState, String(cardId), enabled),
            cardId,
            false
        );
    }
    ...
    function setSidebarAnchor(cardId, column, row) {
        return root.commit(
            CardState.setSidebarAnchor(
                root.internalState, String(cardId), column, row),
            cardId,
            false
        );
    }
    ...
    function setSidebarLayout(layout) {
        ...
        layout.forEach(function(tile) {
            ...
            if (current.column !== column || current.row !== row) {
                next = CardState.setSidebarAnchor(
                    next, tile.id, column, row);
                changed = true;
            }
        });
    ...
    function desktopIdsForScreen(screenName, screenNames) {
        return CardState.desktopIdsForScreen(
            root.state, String(screenName || ""), screenNames || []);
    }

    function resolvedScreenName(cardId, screenNames) {
        return CardState.resolvedScreenName(
            root.state, String(cardId), screenNames || []);
    }

    function sidebarAnchor(cardId) {
        const current = root.card(cardId);
        return current && current.sidebar
            ? { column: current.sidebar.column, row: current.sidebar.row }
    ...
    function setSidebarForeground(active) {
        if (active)
            ...
            root.sidebarConsumerCount = Math.max(
                0, root.sidebarConsumerCount - 1);
        root.syncMonitorOwnership();
    }
