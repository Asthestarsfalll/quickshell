...
class ClavisManagerTest(unittest.TestCase):
    def make_partial_release(
        ...
        systemd = partial / "share/clavis/systemd/user"
        (systemd / "clavis-shell.service").write_text(
            "[Unit]\nRequisite=niri.service\nPartOf=niri.service\nAfter=niri.service\n"
            "[Service]\nExecStart=@CLAVIS_KEY@ shell --foreground --no-duplicate\n"
            "Restart=on-failure\n[Install]\nWantedBy=niri.service\n"
        )
        (systemd / "clavis-clipboard.service").write_text(
            "[Unit]\nRequisite=niri.service\nPartOf=niri.service\nAfter=niri.service\n"
            "[Service]\nExecStart=@CLAVIS_KEY@ clipboard watch\nRestart=on-failure\n"
            "[Install]\nWantedBy=niri.service\n"
        )
    ...
    def test_removed_commands_are_not_routed(self) -> None:
        with tempfile.TemporaryDirectory(prefix="clavis-command-test.") as name:
            with EnvironmentFixture(Path(name)):
                for command in ("run", "prompt", "export", "session"):
                    with self.assertRaises(SystemExit) as stopped:
        ...
        with tempfile.TemporaryDirectory(prefix="clavis-unit-migration-test.") as name:
            ...
            with fixture as paths:
                ...
                installed = {
                    ...
                    for record in manager.load_manifest(paths)["userUnits"]
                }
                self.assertEqual(installed, set(manager.MANAGED_NIRI_USER_SERVICES))

    def test_niri_services_use_stable_key_and_enable_without_starting(self) -> None:
        with tempfile.TemporaryDirectory(prefix="clavis-unit-test.") as name:
            fixture = EnvironmentFixture(Path(name))
            with fixture as paths:
                partial, _launcher = self.make_partial_release(paths, "2026.08.01")
                payloads = manager.user_unit_payloads(paths, partial)
                rendered = {path.name: content.decode() for path, content in payloads}
                self.assertIn(
                    f'ExecStart="{paths.stable_key}" shell --foreground --no-duplicate',
                    rendered["clavis-shell.service"],
                )
                self.assertIn(
                    f'ExecStart="{paths.stable_key}" clipboard watch',
                    rendered["clavis-clipboard.service"],
                )
                self.assertIn("PartOf=niri.service", rendered["clavis-shell.service"])
                os.environ.pop("CLAVIS_SKIP_SYSTEMD", None)
                completed = subprocess.CompletedProcess([], 0, "", "")
                with (
                    mock.patch.object(manager.shutil, "which", return_value="/usr/bin/systemctl"),
                    mock.patch.object(manager.subprocess, "run", return_value=completed) as run,
                ):
                    manager.enable_niri_user_units()
                self.assertEqual(
                    run.call_args.args[0],
                    [
                        "systemctl", "--user", "enable",
                        "clavis-shell.service", "clavis-clipboard.service",
                    ],
                )
