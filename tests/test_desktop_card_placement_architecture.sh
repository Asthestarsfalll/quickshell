...
require_text "$host" 'state.desktop[coordinateSpace].xNorm'
...
require_text "$host" 'SystemCardService.isScreenLayoutMode('
require_text "$host" 'function scheduleDesktopLayout(reason, priorityId)'
require_text "$host" 'property bool layoutScheduled: false'
require_text "$host" 'Qt.callLater(function() {'
require_text "$host" 'window.scheduleDesktopLayout("delegate-ready");'
...
# Screen anchor layouts and runtime-only dragged-card avoidance share the same
# deterministic rectangle solver and do not require wallpaper analysis.
require_text "$solver" 'function solveScreen('
require_text "$solver" 'function resolveDraggedCollision('
require_text "$solver" 'var desktopCardGap = 12;'
require_text "$canvas" 'property var collisionPreviewPositions: ({})'
require_text "$canvas" 'function updateCollisionPreview('
require_text "$desktop_card" 'setDesktopScreenPositions('
require_text Modules/SystemCards/SystemCardCatalog.js 'excludeHostBlur: true'
require_text Modules/ControlCenter/GeneralSidebarPage.qml 'screenTopLeft'
require_text Modules/ControlCenter/GeneralSidebarPage.qml 'screenBottomRight'
