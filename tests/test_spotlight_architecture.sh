...
require_text "$results_panel" 'required property string clipboardEntryId'
require_text "$results_panel" 'const currentResults = root.results;'
...
require_text "$clipboard_provider" 'const entry = root.mergedEntry(source[index]);'
require_text "$clipboard_provider" \
    'return detail ? Object.assign({}, entry, detail) : entry;'
require_text "$clipboard_provider" 'function singleFileSubtitle(file)'
require_text "$clipboard_provider" 'function humanReadableSize(value)'
require_text "$clipboard_provider" 'category === "folder"'
require_text "$clipboard_provider" 'category === "code"'
...
reject_text "$clipboard_provider" 'clipboardModel.append({ modelData:'
reject_text "$clipboard_provider" 'setProperty(resultIndex'
require_text "$clipboard_provider" \
    'if (String(root.query || "").trim() !== "")'
reject_text "$clipboard_provider" 'const useInspectedDetails = needle !== "";'
require_text "$clipboard_service" 'function pruneDetails(entries)'
require_text "$clipboard_service" 'root.pruneDetails(nextEntries);'
require_text "$clipboard_service" 'const stillListed = (root.entries || []).some('
