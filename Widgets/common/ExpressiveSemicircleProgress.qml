...
// Preserves the former Dashboard Hole animated semicircular progress
// treatment for the System media card. Adapted from Caelestia Shell's
// CircularProgress and WavyLine (GPL-3.0).
Item {
    id: root
    ...
    readonly property real arcRadius: Math.max(1, (size - lineWidth * (1 + amplitudeMultiplier * 2)) / 2)
    readonly property real gapAngle: (spacing + lineWidth) / arcRadius
    readonly property real remainingSweepDegrees: Math.max(1 / 360, 180 * (1 - visualProgress) - gapAngle * 180 / Math.PI)
    ...
    Canvas {
        ...
        onPaint: {
            ...
            for (let index = 0; index <= sampleCount; ++index) {
                const theta = startAngle + index * stepAngle;
                const distance = index * stepAngle * radius;
                const waveAngle = root.waveFrequency * Math.PI * 2 * distance / arcLength + phaseRadians;
        ...
        Connections {
            function onVisualProgressChanged() {
            ...
            function onWaveFrequencyChanged() {
                progressCanvas.requestPaint();
            }

            function onAmplitudeMultiplierChanged() {
                progressCanvas.requestPaint();
            }
